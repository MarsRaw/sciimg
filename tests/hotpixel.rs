...
fn test_hot_pixel_correction() {
    ...
    assert_eq!(hpc_results_2p0.replaced_pixels.len(), 21227);

    let hpc_results_2p5 = hotpixel::hot_pixel_detection(&img, 6, 2.5).unwrap();
    assert_eq!(hpc_results_2p5.replaced_pixels.len(), 8401);
    ...
    assert_eq!(hpc_results_2p5_2nd_pass.replaced_pixels.len(), 2013);
}
